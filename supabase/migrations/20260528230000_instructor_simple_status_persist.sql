-- Instructor simple status (מתוכנן / בוצע / בוטל): allow after session start, including before session end.
-- Removes legacy "Cannot mark completed before session end" that blocked persistence.
...
COMMENT ON FUNCTION public.enforce_instructor_session_update() IS
  'Instructor may set planned/completed/cancelled after session start; lead or substitute via instructor_owns_session';
